document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderActivityCard(name, details) {
    ...
    shareButtons.forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        handleSocialShare(button.classList, activityName, details);
      });
    });
  ...
  function handleSocialShare(classList, activityName, details) {
    const pageUrl = window.location.href;
    const formattedSchedule = formatSchedule(details);
    const shareTitle = `Check out ${activityName} at Mergington High School!`;
    const shareText = `${details.description} - Schedule: ${formattedSchedule}`;
